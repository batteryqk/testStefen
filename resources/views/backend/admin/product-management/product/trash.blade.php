<x-admin::layout>
    <x-slot name="title">{{ __('Trashed Product List') }}</x-slot>
    <x-slot name="breadcrumb">{{ __('Trashed Product List') }}</x-slot>
    <x-slot name="page_slug">product</x-slot>
    <section>

        <div class="glass-card rounded-2xl p-6 mb-6">
            <div class="flex items-center justify-between">
                <h2 class="text-xl font-bold text-text-black dark:text-text-white">{{ __('Trashed Product List') }}</h2>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', () => {
                ...
                const details = {
                    ...
                    displayLength: 10,
                    main_route: "{{ route('pm.product.trash') }}",
